...
from teaspn_server.handler_impl_demo import TeaspnHandlerImplDemo
from teaspn_server.protocol import Diagnostic, Position, Range, TextDocumentSyncKind
...
class TeaspnServer(MethodDispatcher):
    ...
    def m_text_document__did_change(self, textDocument=None, contentChanges=None, **_kwargs):
        for change in contentChanges:
            ...
            # self._handler.update_document(range='', text=change['text'])
        diagnostics = self._handler.get_diagnostics()
    ...
    def m_text_document__code_action(self, textDocument=None, range=None, context=None, **_kwargs):
        ...
        if context is not None and context.get('diagnostics', []):
            # code action for resolving a diagnostics -> invoke quick fix
...
def main():
    ...
    if args.tcp:
        bind_addr = '127.0.0.1'
        port = '9520'
        server = socketserver.TCPServer((bind_addr, port), TeaspnServer)
        server.allow_reuse_address = True

    else:
        stdin, stdout = _binary_stdio()
        handler = TeaspnHandlerImplDemo()
